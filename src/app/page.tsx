...
export default async function Home() {
  ...
  // Compute isStale once per server render to avoid hydration mismatch.
  // eslint-disable-next-line react-hooks/purity -- async RSC render: the value is fixed before HTML is sent, so hydration is safe
  const now = Date.now();
  const reposWithStale: RepoWithStale[] = repos.map((repo) => ({
    ...repo,
    isStale: now - new Date(repo.pushed_at).getTime() > STALE_THRESHOLD_MS,
  }));
  ...
  for (const repoWeeks of allActivity) {
    for (const week of repoWeeks) {
      ...
      if (existing) {
        ...
        weekMap.set(week.week, [...week.days] as [
          number,
          number,
          number,
          number,
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-8 lg:gap-12">
          <aside className="lg:sticky lg:top-4 lg:self-start lg:pr-4 flex flex-col gap-3">
            {profile && (
              <Header profile={profile} totalStars={stats.totalStars} />
            )}
            <ContactCard />
            <SocialLinks />
            ...
            <div className="flex items-start justify-between gap-8 mb-10">
              <div className="space-y-3">
                ...
                <p className="text-neutral-600 leading-relaxed max-w-lg">
                  Focused on IT infrastructure, Software-driven automation, and
                  building scalable technical solutions.
                </p>
              </div>
