...
describe('RepoCard', () => {
  it('renders repo name', () => {
    render(<RepoCard repo={freshRepo} />);
  ...
  it('renders stale badge when pushed > 90 days ago', () => {
    render(<RepoCard repo={staleRepo} />);
    expect(screen.getByText('stale')).toBeDefined();
  });

  it('does not render stale badge when pushed < 90 days ago', () => {
    render(<RepoCard repo={freshRepo} />);
    expect(screen.queryByText('stale')).toBeNull();
  });

  it('renders homepage link when present', () => {
    render(<RepoCard repo={repoWithHomepage} />);
    expect(screen.getByText('Live')).toBeDefined();
  });

  it('does not render homepage link when null', () => {
    render(<RepoCard repo={repoNoHomepage} />);
    expect(screen.queryByText('Live')).toBeNull();
  });

  it('renders archived badge when archived', () => {
    render(<RepoCard repo={archivedRepo} />);
