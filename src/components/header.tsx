      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div
              key={stat.label}
              className="flex flex-col border border-neutral-200 rounded-md px-2 py-2.5"
            >
              <span className="text-xl font-bold text-black leading-none">
                {stat.icon && (
                  <span className="text-neutral-400 mr-0.5">{stat.icon}</span>
                )}
                {stat.value}
              </span>
              <span className="text-[10px] font-mono uppercase tracking-wide text-neutral-400 mt-1.5 leading-tight">
                {stat.label}
              </span>
            </div>
          ))}
