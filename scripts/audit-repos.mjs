...
function printReport(results) {
  ...
  const hdr = [
    pad('Repository', PAD.name),
    pad('Language', PAD.lang),
    pad('Stars', PAD.stars),
    pad('Score', PAD.score),
    'Enrichment',
  ].join('  ');
  ...
  console.log('\n  Next steps:\n');
  console.log(
    '     1. Add topics via GitHub UI:  https://github.com/<owner>/<repo>/edit',
  );
  console.log(
    '     2. Add a description & homepage on the same settings page.',
  );
  console.log('     3. Re-run this script to verify:  npm run audit:repos\n');

  const totalFields = results.length * ENRICHABLE_FIELDS.length;
  const filledFields =
    totalFields - incomplete.reduce((sum, r) => sum + r.missing.length, 0);
  const pct =
    totalFields > 0 ? Math.round((filledFields / totalFields) * 100) : 100;
  console.log(
    `  Overall enrichment: ${filledFields}/${totalFields} fields filled (${pct}%)\n`,
  );
}
...
async function main() {
  ...
  if (needsWork > 0) {
    console.log(
      `  WARNING: ${needsWork} repo(s) need enrichment. Exiting with code 1.`,
    );
    process.exit(1);
  }
